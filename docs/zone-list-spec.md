...
Short answer: **no, not for the static-traffic placement workflow** (use `BrnTraffic::Pvs`); the Bundle-Manager PVS-editor parity has since shipped as `ZoneListOverlay`.
...
**For replacing Bundle-Manager's PVS editor** — done. The ZoneList is what that screenshot shows: 428 polygonal zones tagged with a `zoneType` and an explicit safe/unsafe neighbour graph for streaming. `ZoneListOverlay` ([`src/components/schema-editor/viewports/ZoneListOverlay.tsx`](../src/components/schema-editor/viewports/ZoneListOverlay.tsx)) — the overlay mirroring `AISectionsOverlay` — renders the 428 polygonal zones plus a safe/unsafe/IMMEDIATE neighbour graph on the X-Z plane with click-to-select-zone (via the schema NodePath codec), and a bulk-transform gizmo (issue #79). The gizmo's transform runs through the shared transform core's zone-list resolver ([`src/lib/core/transform/resolvers/zoneList.ts`](../src/lib/core/transform/resolvers/zoneList.ts)), where a whole-zone reference expands into one slot per corner so a rotate turns the quad. Steward already parses + writes the resource byte-exact, so the editor is a pure rendering / interaction layer with no new format work.
