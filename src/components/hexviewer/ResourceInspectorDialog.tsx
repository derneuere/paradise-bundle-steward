...
export const ResourceInspectorDialog: React.FC<ResourceInspectorDialogProps> = ({ inspected, onOpenChange, bytesPerRow }) => {
  ...
  // Build entry and header masks for grouping boundaries
  ...
  // Decoding helpers for hover tooltip
  const decodePreview = useMemo(() => {
    function toHex(bytes: Uint8Array): string {
      return Array.from(bytes).map(b => b.toString(16).toUpperCase().padStart(2, '0')).join(' ');
    }
    function toAscii(bytes: Uint8Array): string {
      const str = Array.from(bytes).map(b => (b >= 32 && b <= 126) ? String.fromCharCode(b) : '.').join('');
      return str.replace(/\.+$/, '');
    }
    ...
    function readU64LE(bytes: Uint8Array): string {
    ...
    function decodeCString(bytes: Uint8Array): string {
      ...
      const effective = nul >= 0 ? bytes.subarray(0, nul) : bytes;
  ...
  // Precompute tooltip for each offset (only for selected field segments)
  ...
  const getRow = (rowIndex: number): HexRow => {
    ...
    const hexBytes = Array.from(rowData).map((b, i) => {
      ...
      // Subtle fill for selected field
      ...
      // Field boundaries (inner grouping)
    ...
    // Build grouping wrappers for this row
    ...
    if (schema && headerInfo) {
      ...
      // Header group segment within this row (may span multiple rows)
      if (rowStart < headerSize) {
        ...
        if (e > s) {
          ...
          const cls: string[] = [
            'border-2', 'border-sky-500/60', 'hover:border-sky-400/80', 'p-0.5', '-m-0.5'
          ];
          ...
          groups.push({
            ...
            colStart: s,
            colEnd: e,
            title: `Header bytes [0x${rowStart.toString(16).toUpperCase()}..)` ,
            classes: cls.join(' ')
          });
        }
      }

      // Entry groups that overlap this row
      for (let i = 0; i < count; i++) {
        ...
        if (rowStart < ee && rowEnd > es) {
          ...
          // Determine whether this segment connects to previous/next rows
          const topOpen = es < rowStart;
          const bottomOpen = ee > rowEnd;
          const cls: string[] = [
            'border-2', 'border-amber-500/60', 'hover:border-amber-400/80', 'p-0.5', '-m-0.5'
          ];
          cls.push(topOpen ? 'border-t-0' : 'border-t-2');
          cls.push(bottomOpen ? 'border-b-0' : 'border-b-2');
          if (topOpen) cls.push('-mt-px');
          ...
          groups.push({
            ...
            colStart,
            colEnd,
            title: `Entry ${i} [0x${es.toString(16).toUpperCase()}..0x${(ee-1).toString(16).toUpperCase()}]`,
            classes: cls.join(' ')
          });
        }
      }
        ...
        {inspected && (
          <div className="space-y-3">
            ...
            <HexTable
              ...
              getRow={(i) => {
                const row = getRow(i);
                // Populate multiple decoded items for any fields that intersect this row
                if (inspected && schema && headerInfo) {
                  ...
                  for (const field of schema.fields) {
                    for (let idx = 0; idx < count; idx++) {
                      const fs = headerSize + idx * entrySize + field.offset;
                      const fe = fs + field.size;
                      if (rowStart < fe && rowEnd > fs) {
                        const view = inspected.data.subarray(fs, Math.min(fe, inspected.data.length));
                        const decoded = decodePreview(view);
                        // Add labeled decoded item (include entry index to disambiguate)
                        items.push({ label: `${field.name} #${idx}`, value: decoded });
                  ...
                  if (items.length > 0) {
                    ...
                    // Keep a concise single-line fallback text as well
              ...
              onClickByte={(offset) => {
                ...
                const { headerSize, count } = headerInfo;
                const entrySize = schema.entrySize;
                // If click is inside header, clear selection
                if (offset < headerSize) {
                  setSelectedFieldKey(null);
                  return;
                }
                // Determine position within an entry
                ...
                // Find field covering this relative offset
