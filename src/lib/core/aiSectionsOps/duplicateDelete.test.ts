// Unit tests for duplicateSectionThroughEdge + deleteSection (V12 retail).
//
// Pure-model tests — no fixtures, no React, no IO. We hand-build a tiny
// `ParsedAISectionsV12` and check the geometry / portal wiring of the
// result. Imports come from the directory barrel so we exercise the same
// public surface every downstream caller hits.
...
import { makeModel, makeSection, portalTo, resetPair } from './_testHelpers';
