// V12 retail duplicate-through-edge and delete-section ops.
//
// `duplicateSectionThroughEdge` clones a section's polygon perpendicular
// to one of its edges and wires up a mirrored portal pair so the two
// sections are AI-connected. `deleteSection` removes a section and
// re-threads every cross-reference (portal `linkSection`,
// `sectionResetPairs`) so the model stays internally consistent.
//
// Legacy V4/V6 duplicate lives in `legacy.ts` — it shares the geometry
// math via the helpers module but has its own storage adapter.
...
import { centroid, v2add, v2dot, v2len, v2perp, v2scale, v2sub } from './_helpers';
